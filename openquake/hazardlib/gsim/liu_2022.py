...
"""
Module exports: :class:`LiuEtAl2022SInter`,
                :class:`LiuEtAl2022SSlab`,
                :class:`LiuEtAl2022M1SSlab`,
                :class:`LiuEtAl2022M1SInter`,
                :class:`LiuEtAl2022M2SSlab`,
                :class:`LiuEtAl2022M2SInter`,
...
def get_median_conditional_cav(
    ...
    cav_est = (
        C["a0"]
        # + C["a1"] * np.log(ctx.vs30)
        # + C["a2"] * ctx.mag
        # + C["a3"] * np.log(median_gms["PGA"])
...
#     """
#     # return np.sqrt(sig_cav_cond**2.0 + (sig_pga**2.0) * (C["c2"] ** 2.0))
#     return np.sqrt(sig_cav_cond**2.0 + (sig_pga**2.0) * (C["a1"] ** 2.0))
...
def get_standard_deviations(
    ...
    # sigma_cav_cond = 0.36 if kind == "sinter" else 0.33
...
class LiuEtAl2022SSlab(ConditionalGMPE):
    ...
    def compute(
        ...
        # print("sigma SA(1.0)")
        # print(sigma_gms["SA(1.0)"][0])
        # print("tau SA(1.0)")
        # print(tau_gms["SA(1.0)"][0])
        # print("phi SA(1.0)")
        # print(phi_gms["SA(1.0)"][0])
        ...
        # conv_fact = np.log(1 / 9.81)
        # breakpoint()
        # median_gms_pre, sigma_gms, tau_gms, phi_gms = (
    ...
    # cav         1.50	       0.63	       0.49	     -0.23       0.12	     0.17      0.25
    COEFFS = CoeffsTable(
        table="""\
    IMT                        a0                       a1                      a2                     a3                      a4                       tau                    phi
...
class LiuEtAl2022SInter(LiuEtAl2022SSlab):
    ...
    # IMT           a0           a1          a2         a3         a4        tau       phi
    # cav         2.37	       0.62	       0.41	     -0.27       0.14	    0.14      0.24
    COEFFS = CoeffsTable(
        table="""\
    IMT                       a0                        a1                      a2                     a3                      a4                       tau                    phi
...
def get_median_values_M1(kind, C, ctx, a1100=None):
    ...
    # f_site = C["theta7"]*np.log(ctx.vs30/CONSTANTS["Vlin"]) + CONSTANTS["k2"]*\
    #     np.log(ctx.cav1100+CONSTANTS["c"]*np.log((ctx.vs30/CONSTANTS["Vlin"])**CONSTANTS["n"]))-\
    #         CONSTANTS["k2"]*np.log(ctx.cav1100+CONSTANTS["c"])

    # f_site[ctx.vs30>=CONSTANTS["Vlin"]] = C["theta7"] + CONSTANTS["k2"] * np.log(ctx.vs30[ctx.vs30>=CONSTANTS["Vlin"]]/CONSTANTS["Vlin"])
...
def get_median_values_M2(kind, C, ctx, a1100=None):
    ...
    # f_site = C["a12"]*np.log(temp_vs30/CONSTANTS["Vlin"]) - CONSTANTS["b"]*np.log(ctx.cav1100+CONSTANTS["c"]) + \
    #    CONSTANTS["b"] * np.log(ctx.cav1100+CONSTANTS["c"]*np.log((temp_vs30/CONSTANTS["Vlin"])**CONSTANTS["n"]))

    # f_site[temp_vs30 >= CONSTANTS["Vlin"]] = (C["a12"] + CONSTANTS["b"]*CONSTANTS["n"]) * np.log(temp_vs30[temp_vs30 >= CONSTANTS["Vlin"]]/CONSTANTS["Vlin"])

    f_site = _get_shallow_site_response_term(
        C, temp_vs30, a1100, C["a12"], CONSTANTS["b"]
    )
    # f_site = _get_shallow_site_response_term(C, vs30, cav_rock)
...
def _get_shallow_site_response_term(C, vs30, cav_rock, o_const_1, o_const_2):
    ...
    # f_site_g = (C["theta7"] + CONSTANTS["k2"] * CONSTANTS["n"]) * np.log(vs_mod)
    idx = vs30 < CONSTANTS["Vlin"]
    # print("Vs30 = {}".format(vs30[0]))
    # breakpoint()
    ...
    # breakpoint()
    # print("2nd Vs30 = {}".format(vs30[0]))
    # breakpoint()

    return f_site_g


# def _get_shallow_site_response_term(C, vs30, cav_rock):
#     """
#     Returns the shallow site response term defined in equations 17, 18 and
#     19
#     """
#     vs_mod = vs30 / CONSTANTS["Vlin"]
#     # Get linear global site response term
#     f_site_g = C["theta7"] * np.log(vs_mod)
#     idx = vs30 > CONSTANTS["Vlin"]
#     f_site_g[idx] = f_site_g[idx] + (CONSTANTS["k2"] * CONSTANTS["n"] * np.log(vs_mod[idx]))
#
#     #print("1st Vs30 = {}".format(vs30[0]))
#     #breakpoint()
#
#     # Get nonlinear site response term
#     idx = np.logical_not(idx)
#     if np.any(idx):
#         f_site_g[idx] = f_site_g[idx] + CONSTANTS["k2"] * (np.log(cav_rock[idx] + CONSTANTS["c"] * (vs_mod[idx] ** CONSTANTS["n"])) - np.log(cav_rock[idx] + CONSTANTS["c"]))
#
#     print("2nd Vs30 = {}".format(vs30[0]))
#     breakpoint()
#
...
# def get_median_values(SJ, C, ctx, a1100=None):
#     """
#     Returns the median values for a specific IMT
#     """
#     # Site model defined
#     temp_vs30 = ctx.vs30
#
...
class LiuEtAl2022M1SSlab(GMPE):
    ...
    def compute(self, ctx: np.recarray, imts, median, sig, tau, phi):
        ...
        # cav1100 = get_median_values_M1(C_CAV, ctx)
    ...
    # old
    # changes in theta5, theta 7, theta_nft1, mag_B
    ...
    # cav        2.48      -1.24      0.028       1.56      0.74   -0.002       0.93      0.003         0          0.84         0.20      50   80     -0.008      7.2      0.32   0.57
    # check where used: theta10, Z_ref, Z_B
    # new in erratum
    # IMT      theta1     theta2     theta3     theta4    theta5   theta6     theta7     theta9   theta10    theta_nft1   theta_nft2   Z_ref  Z_B  delta_Z_B    mag_B       tau    phi
    # cav         2.8      -1.24      0.028       1.56      0.75   -0.002       0.94      0.003         0         0.844         0.20      50   80     -0.008      7.6      0.32   0.57
    COEFFS = CoeffsTable(
        table="""\
    IMT      theta1            theta2           theta3          theta4        theta5          theta6           theta7        theta9   theta10        theta_nft1   theta_nft2   Z_ref  Z_B       delta_Z_B    mag_B       tau    phi
...
class LiuEtAl2022M1SInter(LiuEtAl2022M1SSlab):
    """Liu et al. (2022) GMPE for application to
    subduction in-slab earthquakes
    """
    ...
    # old
    # IMT      theta1     theta2     theta3     theta4    theta5   theta6     theta7     theta9   theta10     theta_nft1  theta_nft2     Z_ref  Z_B     delta_Z_B    mag_B       tau    phi
    # cav        1.96      -1.25      0.028       1.42      0.74   -0.002       0.93      0.025         0           0.84        0.20        15   30          0.11      8.4      0.32   0.57
    # changes in theta5, theta 7, theta_nft1, mag_B
    # new in erratum
    # IMT      theta1     theta2     theta3     theta4    theta5   theta6     theta7     theta9   theta10     theta_nft1  theta_nft2     Z_ref  Z_B     delta_Z_B    mag_B       tau    phi
    # cav        1.96      -1.25      0.028       1.42      0.75   -0.002       0.94      0.025         0          0.844        0.20        15   30          0.11      7.9      0.32   0.57
    DEFINED_FOR_TECTONIC_REGION_TYPE = const.TRT.SUBDUCTION_INTERFACE
    kind = "sinter"
    COEFFS = CoeffsTable(
        table="""\
...
class LiuEtAl2022M2SSlab(GMPE):
    ...
    def compute(self, ctx: np.recarray, imts, median, sig, tau, phi):
        ...
        # cav1100 = get_median_values_M2(self.kind, C_CAV, ctx)
    ...
    # old
    # cav        5.85      -1.25       0.03       1.09      0.57   -0.0007        0.19        0.61    0.0003         2    -0.03     -0.032      9.92      7.2     0.38      0.59
    # changes in a1, a2, a4, a5, a6, a9, a10, a11, a13, a14, C4, mag_B, phi
    # added a12
    # not in table: theta7
    # new in erratum
    # IMT          a1         a2         a3         a4        a5        a6          a9         a10       a11    a12        a13       a14       C4    mag_B       tau       phi
    # cav        5.68      -1.17       0.03       0.87      0.59   -0.0017       0.086       1.139    0.0005   0.77     -0.071     -0.12       10      7.5      0.38      0.58
    COEFFS = CoeffsTable(
        table="""\
    IMT                a1                   a2                   a3                    a4                   a5                          a6                       a9                        a10                       a11                      a12                       a13                      a14       C4    mag_B                      tau                     phi
...
class LiuEtAl2022M2SInter(LiuEtAl2022M2SSlab):
    """Liu et al. (2022) GMPE for application to
    subduction in-slab earthquakes
    """
    ...
    DEFINED_FOR_TECTONIC_REGION_TYPE = const.TRT.SUBDUCTION_INTERFACE
    # old
    # cav        5.85      -1.25       0.03       1.09      0.57   -0.0007        0.19       0.61    0.0003         2    -0.03   -0.032       9.92      8.4     0.38      0.59
    # changes in a1, a2, a4, a5, a9, a10, a11, a13, a14, C4, mag_B, phi
    # added a12
    # new in erratum
    # IMT          a1         a2         a3         a4        a5        a6          a9        a10       a11    a12      a13        a14       C4    mag_B       tau       phi
    # cav        5.68      -1.17       0.03       0.87      0.59   -0.0017       0.086      1.139     0.005   0.77   -0.071      -0.12       10      7.8      0.38      0.58
    kind = "sinter"
    COEFFS = CoeffsTable(
        table="""\
    IMT                a1                   a2                   a3                    a4                   a5                          a6                       a9                        a10                       a11                      a12                       a13                      a14       C4    mag_B                      tau                     phi
